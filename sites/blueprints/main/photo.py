...
from flask import render_template, request, current_app, send_from_directory, flash, redirect, url_for, abort
from flask_login import login_required, current_user

from sites.extensions import db
from sites.forms.main import DescriptionForm, TagForm, CommentForm
from sites.models import Photo, Tag, PhotoComment, Collect
from sites.decorators import confirm_required, permission_required
from sites.utils import flash_errors
from sites.notifications import push_collect_notification, push_comment_notification
...
def show_photo(photo_id):
    ...
    pagination = PhotoComment.query.with_parent(photo).order_by(PhotoComment.timestamp.asc()).paginate(page, per_page)
...
def new_comment(photo_id):
    ...
    if form.validate_on_submit():
        ...
        comment = PhotoComment(body=body, author=author, photo=photo)

        replied_id = request.args.get('reply')
        if replied_id:
            comment.replied = PhotoComment.query.get_or_404(replied_id)
