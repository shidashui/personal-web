...
from sites.forms.admin import EditProfileAdminForm
from sites.models import User, Photo, Tag, PhotoComment, Role
from sites.decorators import permission_required, admin_required
from sites.utils import redirect_back
...
def index():
    ...
    comment_count = PhotoComment.query.count()
    reported_comments_count = PhotoComment.query.filter(PhotoComment.flag>0).count()
...
def manage_comment(order):
    ...
    order_rule = 'flag'
    if order == 'by_time':
        pagination = PhotoComment.query.order_by(PhotoComment.timestamp.desc()).paginate(page, per_page)
        order_rule = 'time'
    else:
        pagination = PhotoComment.query.order_by(PhotoComment.flag.desc()).paginate(page, per_page)
